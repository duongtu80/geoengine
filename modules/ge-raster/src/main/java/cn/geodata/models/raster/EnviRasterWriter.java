...
public class EnviRasterWriter {
	...
	public void outputEnvi(int val) throws IOException{
		...
		if(this.dataType == 12 || this.dataType == 2){
...
//		IOUtils.write(_bs, this.fileStream);
	...
	public void writeHeader(String interleave, List<String> bands, String mapInfo, Map<String, Object> params) throws IOException {
...
//				"0.0, 0.0, 29.500000, 45.500000, North America 1983, Albers Conical Equal Area, units=Meters}");
		...
		_lines.add("byte order = 0");
		_lines.add("band names = {" + Arrays.toString(bands.toArray()).replaceAll("[\\[\\]]", "") + "}");
		
		if(params != null){
			for(String _key : params.keySet()){
				Object _val = params.get(_key);
				
				if (_val instanceof String) {
					String _txt = (String) _val;
					_lines.add(_key + " = \"" + _txt + "\"");
				}
				else if(_val instanceof Number){
					Number _txt = (Number) _val;
					_lines.add(_key + " = " + _txt.toString());
				}
				else{
					log.info("Unkown parameter type " + _key + " (" + _val.getClass().toString() + ")");
				}
			}
		}
