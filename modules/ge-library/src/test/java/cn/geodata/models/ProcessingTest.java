...
//		System.out.println(Arrays.toString(args));
		ProcessingTest _p = (new ProcessingTest());
	...
	public void setGlacier(File glacier) {
		this.glacier = glacier;
	}

	public void setBasin(File basin) {
		this.basin = basin;
	}
...
//			System.out.println("Balances:" + Arrays.toString((double[])_library.getOutput(_runoModel, "Balances")));
	...
	private MultiPolygon getCatchment(String catchmentName) throws MalformedURLException, IOException {
		...
		try{
			while(_it.hasNext()){
				Feature _f = _it.next();
				if(_f.getProperty("站点").equals(catchmentName)){
					return (MultiPolygon) _f.getDefaultGeometryProperty();
				}
			}
		}
		finally{
