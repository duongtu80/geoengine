...
public class Operations {
	...
	public String loadParamData() throws Exception {
		...
		for(int i=0;i<_dates.size();i++){
			...
			_items.put(_dates.get(i));
			for(int j=0;j<_outputs.size();j++){
				if(_outputs.get(j).getDates().contains(_dates.get(i))){
					_items.put(_outputs.get(j).getValues().get(_outputs.get(j).getScenarios().get(0)).get(_outputs.get(j).getDates().indexOf(_dates.get(i))).get(this.param));
				}
				else{
					_items.put(-9999);
				}
		...
		_root.put("columns", JSONArray.fromArray(_columns.toArray()));
