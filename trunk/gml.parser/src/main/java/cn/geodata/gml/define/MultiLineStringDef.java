...
public class MultiLineStringDef extends AbstractDefinition {
	...
	public Element encode(Object obj) throws Exception {
		...
		LineStringDef _lineStringDef = new LineStringDef(this.config);
		for(int i=0;i<_lines.getNumGeometries();i++){
			Element _member = new Element("lineStringMember", this.config.getUriGML());
			_member.addContent(_lineStringDef.encode(_lines.getGeometryN(i)));
	...
	public Object parse(Element ele) throws Exception {
		...
		for(Element _lineMemberNode : (List<Element>)ele.getChildren("lineStringMember", this.config.getUriGML())){
			LineString _line = (LineString) _lineStringDef.parse(_lineMemberNode.getChild("LineString", this.config.getUriGML()));
			_lines.add(_line);
		}
