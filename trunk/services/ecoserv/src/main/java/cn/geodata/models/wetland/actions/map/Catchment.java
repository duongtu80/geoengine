...
public class Catchment {
	...
	private FeatureSource<SimpleFeatureType, SimpleFeature> features;
	...
	public MultiPolygon findCatchmentByTag(String tag) throws NumberFormatException, Exception{
		...
		FeatureIterator<SimpleFeature> _it = this.features.getFeatures().features();
		try{
			log.info("tag:" + tag);
			while(_it.hasNext()){
	...
	public SimpleFeature findWetland(Point pt) throws IOException {
		FilterFactory2 _factory = CommonFactoryFinder.getFilterFactory2(GeoTools.getDefaultHints());
		FeatureIterator<SimpleFeature> _it = this.features.getFeatures(_factory.contains(_factory.property(this.features.getSchema().getGeometryDescriptor().getLocalName()), _factory.literal(pt))).features();
		try{
