...
//		if(_list.get(_list.size() - 1).x != _list.get(0).x && _list.get(_list.size() - 1).y != _list.get(0).y){
//			_list.add((Coordinate) _list.get(0).clone());
//		}
//		
...
//			_ring.setName("LinearRing");
	...
	public Element encodeG3(Object obj) throws IOException {
		if(this.canEncode(obj) == false){
			throw new UnsupportedType(obj.toString());
		}
	...
	public Object parse(Element ele) throws IOException {
		...
		Element _exteriorNode = ele.getChild("LinearRing", this.config.getUriGML());
		...
		for(Element _r : (List<Element>) ele.getChildren("LinearRing", this.config.getUriGML())){
	...
	public Object parseG3(Element ele) throws IOException {
		if(this.canParse(ele) == false){
			throw new UnsupportedType("Element " + ele.getName());
		}
