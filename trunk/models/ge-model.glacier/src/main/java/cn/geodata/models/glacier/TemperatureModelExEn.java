...
@GeoProcess(title="Enhanced Temperature Spatial Interpolation", keywords={"temperature", "idw"})
public class TemperatureModelExEn implements Calculate {
	private Logger log = Logger.getLogger(TemperatureModelExEn.class.getName());
...
//		double _elevation = this.demModel.getLocationValue(point.getX(), point.getY());
		...
		try{
			...
			for(Feature _f : (Feature[])_fs.toArray(new Feature[0])){
				Point _pt = (Point) _f.getDefaultGeometryProperty();
				double _elv = _pt.getCoordinate().z; //this.demModel.getLocationValue(_pt.getX(), _pt.getY());
				
				if(_elv > 0){
					double _val = ((Double)_f.getProperty("val").getValue());
					_f.getProperty("val").setValue(_val - (_elv - this.stand) * this.grads);
	...
	@GeoInput(title="Maximum Site Number")
	...
	@GeoInput(title="Temperature Grads")
	public void setGrads(double grads) {
		this.grads = grads;
	}

	@GeoInput(title="Standard Altitude")
