...
//		log.info("Area " + Math.sqrt(this.calculateArea(_c)));
		...
		double _spillPoint = _reader.getRaster().readFloat(_lowestB.getRow(), _lowestB.getCol()) - _reader.getRaster().readFloat(_lowestC.getRow(), _lowestC.getCol());
	...
	public MultiPolygon findCatchmentByTag(FeatureSource catchments, String tag) throws NumberFormatException, Exception{
		...
		try{
			...
			while(_it.hasNext()){
				Feature _f = _it.next();
				String _tag = (String) _f.getAttribute("NAME");
