...
public class WaterRegionCalcGeoTiff {
...
//		log.info("ENV:" + _env2.toString());
		...
		if(_env.getMaxY() > _env2.getMaxY()){
			_extent.setMinY((int) Math.floor((_env.getMaxY() - _env2.getMaxY())/ this.raster.getCellSizeX()));
		}
		if(_env.getMinY() < _env2.getMinY()){
			_extent.setMaxY((int) Math.floor((_env.getMaxY() - _env2.getMinY()) / this.raster.getCellSizeX()));
		}
	...
	private WetlandDemPixel[] getPixelRange(MultiPolygon polygon, Extent extent) throws IOException{
		...
		for(int _row = extent.getMinY();_row <= extent.getMaxY();_row++){
			...
			for(int _col=extent.getMinX(); _col <= extent.getMaxX(); _col++){
				...
				if(_distance <= 0){
					Number _nn = this.raster.getCell(_col, _row);
					if(_nn == null)
						continue;
					
					float _val = _nn.floatValue(); 
					if(_val < 0)
						continue;
