...
public class GlacierRunoffModel1Test extends TestCase {
...
//		Pattern _pattern = Pattern.compile("(\\d+)\\s(\\S+)\\s([\\d\\.]+)\\s([\\d\\.]+)");
	...
	public void testTest3() throws Exception {
		...
		FeatureSource _fs = _dataStore.getFeatureSource("geo:province");
		assertNotNull(_fs);
...
//		FeatureIterator _it = _fs.getFeatures().features();
	...
	public void atestCalculate() throws Exception {
		...
		_model.setCatchmentModel(new CatchmentModel("/glacier/data/catchment/tailang.shp", "NAME"));
		_model.setIceDDFModel(_iceDdfModel);
		_model.setSnowDDFModel(_snowDdfModel);
		_model.setPrepareModel(new GlacierPrepareModel(_demModel, "/glacier/data/catchment/1144G.shp"));
